...
'''Script which generates relative improvement on a per-iteration basis which is per-sample averaged'''


class pure_dice_relative_score_sample_averaged():
    ...
    def relative_score_generation(self, results_dir, filename_base, scores):
        ...
        per_iter_improv_scores = self.compute_per_iter_improvement(just_scores, parametrisation)
        ...
        for i in range(self.num_samples):
            ...
            row = [scores[0][i]]
            score_list = [sublist[i] for sublist in relative_improv_scores]
            for val in score_list:
                row.append(val) 

            with open(os.path.join(results_dir, f'{filename_base}_per_sample_averaged_relative_to_init.csv'),'a') as f:
        ...
        for i in range(self.num_samples):
            #Extracting sample name
            row = [scores[0][i]]
            score_list = [sublist[i] for sublist in per_iter_improv_scores]
            
            for val in score_list:
                row.append(val)

            with open(os.path.join(results_dir, f'{filename_base}_per_sample_averaged_per_iter.csv'),'a') as f:
    ...
    def compute_relative_improvement(self, output_scores, parametrisation):
        ...
        return [np.array(sublist) - initialisation for sublist in output_scores[1:]]
    ...
    def compute_per_iter_improvement(self, output_scores, parametrisation):
        ...
        return [np.array(sublist) - output_scores[i] for i,sublist in enumerate(output_scores[1:])]
    ...
    def __call__(self):
        ...
        self.relative_score_generation(results_per_sample_averaged_dir, f'{self.base_metric}', filtered_and_sample_averaged)
        ...
        if self.per_class_scores:
            ...
            for class_label in config_labels.keys():
                ...
                self.relative_score_generation(results_per_sample_averaged_dir, f'class_{class_label}_{self.base_metric}', filtered_and_sample_averaged)