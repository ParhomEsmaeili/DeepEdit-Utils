...
class test_scores():
    ...
    def extract_scores_per_iteration(,sequentiality_mode):
        if sequentiality_mode == "CIM":
            #In this case, we first need to compute
    ...
    def score_computation(scoring_tools, infer_run_mode, img_directory_path, results_save_dir, weightmap_parametrisations, human_measure, base_metric, per_class_scores, config_labels, sequentiality_mode):
        '''
        ...
        if human_measure.title() == "None":
            #In this case, this is the default behaviour, just compute the scores with the "None" type weightmaps (just generates a tensor of ones for the image mask) 
            ...
            if infer_run_mode[0].title() == "Editing":
                ...
                for image in image_names:
                    ...
                    scores.append(initialisation_score_tool(initialisation_folder, gt_image_folder, image,)) # Adding the initialisation score
                    
                    for iteration_folder in iteration_folders: #Adding the scores on the intermediary iterations
                        scores.append(editing_score_tool(os.path.join(img_directory_path, 'labels', iteration_folder), gt_image_folder, image))
                    
                    #for the final image
                    scores.append(editing_score_tool(final_image_folder, gt_image_folder, image))
                ...
                for image in image_names:
                    scores = [image]
                    scores.append(score_tool(final_image_folder, gt_image_folder, image))
        ...
        if human_measure.title() == "Local Responsiveness":

            #In this case, only the prediction, the ground truth, and the guidance points set, and the parametrisations for the weightmap for the metric are provided. 
    ...
    def __call__(self):
        ...
        self.score_computation(metric_computer_tools,
                               self.infer_run_mode,
