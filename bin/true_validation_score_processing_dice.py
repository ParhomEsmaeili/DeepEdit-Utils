...
def dice_score_computation(img_directory, inference_tasks, results_save_dir, jobs, study_name):
    ...
    if "compute" in jobs:
        ...
        if framework == "deepeditplus":
            framework = "deepeditPlus"

        dataset_name = study_name[:-11]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--datetime", nargs="+", default=["10072024_182402"])
    ...
    parser.add_argument("-ta", "--task", nargs="+", default=["deepeditplusplus","deepedit", "autoseg", "10"], help="The framework selection + subtask/mode which we want to execute")
    parser.add_argument("--app_dir", default = "MonaiLabel Deepedit++ Development/MONAILabelDeepEditPlusPlus")
    parser.add_argument("--job", default= "collecting", help="argument that determines which job is required from the script (i.e. plotting or computing dice scores)")
    # parser.add_argument("--plot_type", default="errorbar")
    parser.add_argument("--rejection_val", default='0', help='Parameter which controls what the failure value is for the dice scores to not include in averages')
    ...
    validation_folder = args.studies
    dataset_name = args.studies[:-10]
    train_folds = '_'.join(args.train_folds)
    train_val_fold_split_string = f'train_folds_{train_folds}_validation_fold_{args.val_fold}'
    ...
    if job == "compute":
        ...
        if len(args.datetime) == 1 and len(args.checkpoint) == 1 and len(args.infer_run) == 1:
            ...
            if inference_task == "deepedit":
                ...
                inference_image_subtask = validation_folder + f'/{framework}/{train_val_fold_split_string}/{inference_task}_{initialisation}_initialisation_numIters_{num_clicking_iters}/{datetime}/{checkpoint}/{run_infer_string}'
                inference_image_subdirectory = 'datasets/' + inference_image_subtask
                inference_image_output_subdirectory = dataset_name + f'/{framework}/{train_val_fold_split_string}/{inference_task}_{initialisation}_initialisation_numIters_{num_clicking_iters}/{datetime}/{checkpoint}/{run_infer_string}'
            else:
                inference_image_subtask = validation_folder + f'/{framework}/{train_val_fold_split_string}/{inference_task}/{datetime}/{checkpoint}/{run_infer_string}'
                inference_image_subdirectory = 'datasets/' + inference_image_subtask
                inference_image_output_subdirectory = dataset_name + f'/{framework}/{train_val_fold_split_string}/{inference_task}/{datetime}/{checkpoint}/{run_infer_string}'
            
            results_save_dir = app_dir + '/validation_results/' + inference_image_output_subdirectory #inference_image_subtask 
    ...
    #This is for collecting the dice scores across the different runs of a single model/single checkpoint.
        ...
        if inference_task == "deepedit":
            ...
            inference_image_subtasks = [f'{framework}/{train_val_fold_split_string}/{inference_task}_{initialisation}_initialisation_numIters_{num_clicking_iters}/{datetime}/{checkpoint}/run_{infer_run}' for infer_run in args.infer_run]
            # inference_image_subdirectory = 'datasets/' + inference_image_subtask
            results_save_dir = app_dir + '/validation_results/' + dataset_name + f'/{framework}/{train_val_fold_split_string}/{inference_task}_{initialisation}_initialisation_numIters_{num_clicking_iters}/{datetime}/{checkpoint}/run_collection/{run_infer_string}'
        else:
            inference_image_subtasks = [f'{framework}/{train_val_fold_split_string}/{inference_task}/{datetime}/{checkpoint}/run_{infer_run}' for infer_run in args.infer_run] 
            # inference_image_subdirectory = 'datasets/' + inference_image_subtask

            results_save_dir = app_dir + '/validation_results/' + dataset_name + f'/{framework}/{train_val_fold_split_string}/{inference_task}/{datetime}/{checkpoint}/run_collection/{run_infer_string}'
        dice_score_files_base_dir = app_dir + '/validation_results/' + dataset_name 
