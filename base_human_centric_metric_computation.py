...
class score_tool():
    def __init__(self, label_names, human_measure, weightmap_types, metric_base, include_background_mask, include_background_metric, ignore_empty, include_per_class_scores):
    
        self.human_measure = human_measure 
        self.weightmap_types = weightmap_types
        ...
        '''
        if self.human_measure.title() == "Local Responsiveness":
        ...
        if any(self.weightmap_types) not in self.supported_click_weightmaps + self.supported_gt_weightmaps:
            raise ValueError("The selected weightmap type is not supported by the mask generator utilities") 
        ...
        self.mask_generator = MaskGenerator(self.click_weightmap_types, self.gt_weightmap_types, human_measure, self.dict_class_codes)
    ...
    def __call__(self, pred_folder_paths, gt_folder_path, image_name, guidance_points_set, guidance_points_parametrisations):
        ...
        if self.human_measure == "Local Responsiveness":
            ...
            #Output dict will contain the extracted pred(s) and gt in RAS orientation. 
            pred_1 = output_dict['pred_1'][0]
            pred_2 = output_dict['pred_2'][0]
            gt = output_dict['gt'][0]
        ...
        output_score = self.scoring_util(image_mask=(cross_class_map, per_class_maps), pred=final_pred,gt=gt)
