############################################
# NOTE: This code assumes that the dataset is in the BraTS2021 format
######################################################################
...
def json_generator(d):
    ...
    json_dict["name"] = "BRATS2021_T2"
    json_dict["modality"] = 'MRI'
...
def image_conversion(d, train_list, test_list):
    ...
    transforms_list = [
        LoadImaged(keys=('image', 'default'), image_only=False),#reader="ITKReader", image_only=False),
...
def dataset_reformatting(d, training_list, test_list):
    ...
    for filename in training_list:
        shutil.copy(os.path.join(d["input_images_dir"], filename, filename + f'_{d["actions_config"][2]}.nii.gz'), os.path.join(d["output_training_dir"], filename + '.nii.gz'))
        shutil.copy(os.path.join(d["input_images_dir"], filename, filename + '_seg.nii.gz'), os.path.join(d["output_training_labels_dir"], filename + '.nii.gz'))
    for filename in test_list:
    #copy the images and labels for the test list
        shutil.copy(os.path.join(d["input_images_dir"], filename, filename + f'_{d["actions_config"][2]}.nii.gz'), os.path.join(d["output_testing_dir"], filename + '.nii.gz'))
        shutil.copy(os.path.join(d["input_images_dir"], filename, filename + '_seg.nii.gz'), os.path.join(d["output_testing_labels_dir"], filename + '.nii.gz'))
...
def dir_generator(d):
    ...
    d["input_images_dir"] = input_base_dir
...
def parse_arguments():
    ...
    parser.add_argument("--studies", nargs=1, default=['BraTS2021_Training_Data', 'Tr'])#['Task09_Spleen', 'Tr']) # help="Folder for the study and the subcategory (e.g Ts or Tr)")
    ...
    parser.add_argument("--channels_extracted", nargs="+", default=['t2'])#['3'], help="List of channel combinations which are desired in the outputted generated datasets, e.g. [[1,2,3], [1,3,4], [1,2]]")
    return parser
